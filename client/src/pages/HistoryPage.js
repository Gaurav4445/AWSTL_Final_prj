...
import { applianceAPI, propertyAPI, recordAPI, taskAPI, vendorAPI } from '../services/api';
...
export const HistoryPage = () => {
  ...
  const [appliancesForProperty, setAppliancesForProperty] = useState([]);
  ...
  useEffect(() => {
    ...
    if (!propertyId) {
      setTasksForProperty([]);
      setAppliancesForProperty([]);
      return;
    }
    Promise.all([taskAPI.getByProperty(propertyId), applianceAPI.getByProperty(propertyId)])
      .then(([taskResponse, applianceResponse]) => {
        setTasksForProperty(taskResponse.data.data || []);
        setAppliancesForProperty(applianceResponse.data.data || []);
      })
      .catch(() => {
        setTasksForProperty([]);
        setAppliancesForProperty([]);
  ...
  const fetchInitial = async () => {
    try {
      setLoading(true);
      const [recordRes, propertyRes, vendorRes] = await Promise.all([recordAPI.getAll(), propertyAPI.getAll(), vendorAPI.getAll()]);
  ...
  const handleInvoiceUpload = (e) => {
    ...
    reader.onload = () => {
      setFormData((prev) => ({
        ...prev,
        invoiceAttachment: { fileName: file.name, dataUrl: reader.result },
      }));
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.serviceDate) return toast.error('Please select service date');
    if (!formData.actualCost) return toast.error('Please enter actual cost');
    ...
    try {
      ...
      ['taskId', 'applianceId', 'vendorId', 'upiTransactionId'].forEach((field) => {
  ...
  if (searchQuery) {
    ...
    filtered = filtered.filter((record) =>
      ...
      record.applianceId?.name?.toLowerCase().includes(query)
    );
  }
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: '#f2ebe1', fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
        ...
        <main style={{ flex: 1, padding: '48px 48px 64px', overflowY: 'auto' }}>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} style={{ marginBottom: 32 }}>
                <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e4ddd4', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
                  <div style={{ padding: '24px 28px', borderBottom: '1px solid #f0e9e0', background: 'linear-gradient(135deg, #1c2b27 0%, #2d4a3e 100%)' }}>
                    ...
                    <p style={{ color: 'rgba(245,237,228,0.6)', fontSize: 13, marginTop: 4, marginBottom: 0 }}>Add a new maintenance or service entry with warranty and invoice proof.</p>
                  </div>
                  <div style={{ padding: 28 }}>
                    <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 20 }}>
                      ...
                      <div><Label htmlFor="applianceId">Appliance</Label><Select id="applianceId" name="applianceId" value={formData.applianceId} onChange={handleChange}><option value="">Select Appliance</option>{appliancesForProperty.map((appliance) => <option key={appliance._id} value={appliance._id}>{appliance.name}</option>)}</Select></div>
                      ...
                      <div><Label htmlFor="invoiceAttachment">Invoice Upload</Label><Input id="invoiceAttachment" type="file" accept=".pdf,image/*" onChange={handleInvoiceUpload} />{formData.invoiceAttachment?.fileName ? <p style={{ margin: '6px 0 0', fontSize: 12, color: '#6b7565' }}>{formData.invoiceAttachment.fileName}</p> : null}</div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} style={{ display: 'flex', gap: 12, marginBottom: 28, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <Select value={filterProperty} onChange={(e) => setFilterProperty(e.target.value)} style={{ maxWidth: '220px' }}><option value="all">All Properties</option>{properties.map((property) => <option key={property._id} value={property._id}>{property.name}</option>)}</Select>
            <Select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} style={{ maxWidth: '220px' }}><option value="all">All Categories</option>{CATEGORIES.map((category) => <option key={category}>{category}</option>)}</Select>
            {(filterProperty !== 'all' || filterCategory !== 'all' || searchQuery) ? <button onClick={() => { setFilterProperty('all'); setFilterCategory('all'); setSearchQuery(''); }} style={{ padding: '9px 16px', borderRadius: 10, background: '#e4ddd4', color: '#6b7565', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit' }}>Clear</button> : null}
          </motion.div>

          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>{[1, 2, 3].map((item) => <Skeleton key={item} className="h-24 rounded-xl" />)}</div>
          ) : filtered.length === 0 ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ background: '#fff', borderRadius: 16, padding: '56px 32px', textAlign: 'center', border: '2px dashed #d8d1c7' }}>
              <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#f2ebe1', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}><Calendar size={24} color="#6b7565" /></div>
                ...
                return (
                  <motion.div key={record._id} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.04, duration: 0.3 }}>
                    <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e4ddd4', padding: '18px 22px', display: 'flex', alignItems: 'center', gap: 18 }}>
                      <img
                        src={propertyImageForType(record.propertyId?.propertyType || 'Apartment')}
                        alt={record.propertyId?.name || 'Property'}
                        style={{ width: 84, height: 84, objectFit: 'cover', borderRadius: 12, flexShrink: 0, border: '1px solid #ece5db' }}
                      />
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                          ...
                          {record.applianceId ? <span style={{ fontSize: 12, color: '#6b7565', background: '#eef2ff', padding: '2px 10px', borderRadius: 20, fontWeight: 500 }}>{record.applianceId.name}</span> : null}
                        </div>
                        ...
                        <p style={{ fontSize: 12, color: '#9b8f85', margin: '4px 0 0' }}>{new Date(record.serviceDate).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                      </div>
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        ...
                        {record.invoiceAttachment?.dataUrl ? <a href={record.invoiceAttachment.dataUrl} target="_blank" rel="noreferrer" style={{ display: 'block', marginTop: 6, fontSize: 12, color: '#1c2b27' }}>View invoice</a> : null}
                      </div>
